...
genRects :: Float -> Float -> Float -> Float -> Float -> [Rect]
genRects  0 w h z y = [(((w*z), y),w,h)]
genRects  n w h z y = [(((w*(z-n)), y), w, h)] : genRects(n-1, w, h, y, z)
...
main = do
  let
    rects = genRects 10 50 50 10 0.0                        -- Deve gerar 10 retangulos de 50x50
    styles = ["fill:rgb(140,75,0)","fill:rgb(75,140,0)"] -- Estilo: vermelho e verde
